<template>
    <div class="flex-1 space-y-2">
        ...
        <div class="flex items-center w-full bg-neutral-900 ring-1 ring-neutral-800/70 focus:ring-violet-800 outline-none rounded overflow-hidden space-x-2" :class="{'pl-2': icon}">
            ...
            <input @input="$emit('update:input', ($event.target as HTMLInputElement).value)" @keyup="onEnter" :value="input" class="w-full bg-neutral-900 outline-none placeholder:text-neutral-600 p-2" :type="type" :placeholder="placeholder" :name="name" :id="id"/>
        </div>
    </div>
</template>
